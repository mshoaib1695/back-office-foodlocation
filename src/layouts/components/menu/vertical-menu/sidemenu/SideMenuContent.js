...
class SideMenuContent extends React.Component {
  ...
  componentDidMount() {
    let newArr = []
    let screensList = [...Object.entries(this.props.screeensList)]
    for (let i = 0; i < screensList.length; i++) {
      let screens = []
      for (let j = 0; j < screensList[i][1].length; j++) {
        let screen = screensList[i][1][j]
        if(
          screen.screenUrl != "/productsetup"
          &&
        
          screen.screenUrl != "/attributesetup"
          &&
          screen.screenUrl != "/onlinesaleorders"
          ){
          let obj = {
            id: screen.id,
            ...
            navLink: "/dashboard"+screen.screenUrl
          }
          screens.push(obj)
        }
      }
      let obj = {
        id: screensList[i][1][0].name,
        title: screensList[i][1][0].name,
        type: "collapse",
    ...
    this.setState({ screeensList: newArr })
  ...
  render() {
    ...
    const menuItems = this.state.screeensList.length > 0 &&
      this.state.screeensList.map(item => {
...
const mapStateToProps = state => {
  return {
    screeensList: state.auth.login.screeensList,
  }
}
