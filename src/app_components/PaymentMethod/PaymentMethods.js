...
import { paymentMethodUpdate } from '../../redux/actions/updatescreens/role'
...
function PaymentReason() {
    ...
    const fetch = ({ pageSize, page, sorted, filtered }) => {
        ...
        gridDataByClient({
            data: payload,
            apiname: "paymentMethodsByClient",
            tokenType: user.tokenType,
            accessToken: user.accessToken,
        })
            .then(res => {
        ...
        <Card>
            <CardHeader>
                ...
                <Button.Ripple
                    ...
                    type="submit"
                    className="mr-1 mb-1"
                    onClick={e => history.push("/dashboard/createpaymentmethod")}
                >
                            ...
                            columns: [
                                {
                                    ...
                                    Cell: (row) => (
                                        <p className="gridBtn"
                                            onClick={() => {
                                                dispatch(paymentMethodUpdate({ ...row.original }))
                                                history.push('/dashboard/updatepaymentmethod')
                                            }}>
                                            {row.original.name}
                                        </p>
                                    )
