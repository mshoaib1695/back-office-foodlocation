...
import { tableUpdate } from '../../redux/actions/updatescreens/role'
...
function Users(props) {
        ...
        if (filtered && filtered.length > 0) {
            for (let index = 0; index < filtered.length; index++) {
                ...
                if (element.id == "tableNo") {
                    payload.tableNo = element.value
                }               
            }
        }
        gridDataByClient({
            data: payload,
            apiname: "flrTableByFloor",
    ...
    return (
        <Card>
            <CardHeader>
                {/* createmenuitem */}
                <CardTitle>Branch Floor Tables</CardTitle>
                <Button.Ripple
                    color="primary"
                    type="submit"
                    className="mr-1 mb-1"
                            ...
                            columns: [
                                {
                                    ...
                                    Cell: (row) => (
                                        <p className="gridBtn"
                                            onClick={() => {
                                                dispatch(tableUpdate({ ...row.original }))
                                                props.goToUpdate()
                                            }}>
                                            {row.original.name}
                                        </p>
