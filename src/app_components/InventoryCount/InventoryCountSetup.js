        ...
        <div className="data-list-header d-flex justify-content-between flex-wrap">
            <div className="actions-left d-flex flex-wrap">
                ...
                <Button
                    ...
                    color="primary"
                    onClick={() => {
                        history.push("/dashboard/createinventory")
                    }}
                    outline>
                    <Plus size={15} />
    ...
    fetch = ({ pageSize, page, sorted, filtered }) => {
        ...
        if (filtered && filtered.length > 0) {
            for (let index = 0; index < filtered.length; index++) {
                ...
                if (element.id == "inventoryDate") {
                    payload.inventoryDate = element.value
                }
        ...
        gridDataByClient({
            data: payload,
            apiname: "inventoryCountsByClient",
            tokenType: this.props.user.tokenType,
            accessToken: this.props.user.accessToken,
        })
            .then(res => { 
    ...
    render() {
        ...
        const columns = [
            ...
            },
            {
                ...
                minWidth: "60px",
                cell: row => (
                    <p title={row.inventoryDate} className="text-truncate text-bold-500 mb-0">
                        {row.inventoryDate}
                    </p>
                )
            },
        
            {
                ...
                cell: row => (
                    <p title={row.warehouse} className="text-truncate text-bold-500 mb-0">
                        {this.state.warehousesList.length > 0 && this.state.warehousesList.filter(i => i.value == row.warehouse).length > 0 &&
                            this.state.warehousesList.filter(i => i.value == row.warehouse)[0].label
                        }
                    </p>
                )
            },
            ...
            },
            {
                ...
                minWidth: "60px",
                cell: row => (
                    <p title={row.description} className="text-truncate text-bold-500 mb-0">
                        {row.description}
                    </p>
                )
            },
...
const mapDispatchToProps = dispatch => {
    return {
        updateHandler: payload => dispatch(inventoryCountUpdate({ ...payload }))
    }
}
