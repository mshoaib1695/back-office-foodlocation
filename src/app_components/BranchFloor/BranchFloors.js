...
import { floorUpdate } from '../../redux/actions/updatescreens/role'
...
function Users(props) {
    ...
    const fetch = ({ pageSize, page, sorted, filtered }) => {
        ...
        if (filtered && filtered.length > 0) {
            for (let index = 0; index < filtered.length; index++) {
                ...
                if (element.id == "floorNo") {
                    payload.floorNo = element.value
                }               
            }
        }
        gridDataByClient({
            data: payload,
            apiname: "floorByBranch",
    ...
    return (
        <Card>
            <CardHeader>
                {/* createmenuitem */}
                <CardTitle>Branch Floors</CardTitle>
                <Button.Ripple
                    color="primary"
                    type="submit"
                    className="mr-1 mb-1"
                            ...
                            columns: [
                                {
                                    ...
                                    Cell: (row) => (
                                        <p className="gridBtn"
                                            onClick={() => {
                                                dispatch(floorUpdate({ ...row.original }))
                                                props.goToUpdate()
                                            }}>
                                            {row.original.name}
                                        </p>
