...
function UpdateRole() {
    const role = useSelector(state => state.updatescreens.role)
    ...
    const submitHandler = (values) => {
        ...
        updateRole(payload)
            .then(res => { message(res)
                ...
                if (res.data.success) {
                    history.push("/dashboard/rolesetup/")
                }
            })
    }
        ...
        <Card>
            ...
            <CardBody>
                                ...
                                <Col sm="12">
                                    <FormGroup className="form-label-group">
                                        ...
                                        <Button.Ripple
                                            ...
                                            type="reset"
                                            className="mb-1"
                                            onClick={() => history.push("/dashboard/rolesetup/")}
                                        >
                                            Cancel
                  </Button.Ripple>
                                    </FormGroup>
                ...
                <AddToListComponent
                    ...
                    data={screenList}
                    fieldTitle={"Assign Screen"}
                    onRemoveToList={(value) => removeFromWarehouseList(value)}
                    onChangeEditable={(row, value) => onChangeEditable(row, value)}
                />
            </CardBody>
        </Card>
    )
