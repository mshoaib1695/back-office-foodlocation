...
import { create, parametersListByParaType, getList } from '../../API_Helpers/api'
...
function CreateRole() {
    ...
    const branch = useSelector(state => state.updatescreens.branch)
    ...
    const getBranchById = (id) => {
        let payload = {
            ...
            apiname: `branchByID?id=${id}&lang=EN`
        }
        getList(payload)
            .then(res => {
                setLocation({ lat: res.data.object.latitude, lng: res.data.object.longitude })
            })
    }
            ...
            <CardBody>
                <Formik
                    initialValues={{
                        ...
                        isMinCharge: branch.isMinCharge,
                        isTaxOnChrg: branch.isTaxOnChrg,
                        isBarcodeEnable: branch.isBarcodeEnable,
                        isTobacco: branch.isTobacco,
                        minCharge: branch.minCharge,
                        isColPointEnable: branch.isColPointEnable,
                        chargeType: branch.chargeType,
                        isInclusiveTax: branch.isInclusiveTax,
                        isPrntKitPayComp: branch.isPrntKitPayComp,
                        pointValueAmt: branch.pointValueAmt,
                        pointValue: branch.pointValue,
                        pointValueAmt: branch.pointValueAmt,
                    }}
