...
import { productWarehouseLowLimitUpdate } from '../../redux/actions/updatescreens/role'
...
function ProductProductOffers(props) {
    ...
    const deleteHandler = (id) => {
        let payload = {
            ...
            apiname: "deleteProdWarehouseLimit"
        }
        deleteapi(payload)
            .then(res => { message(res)
    ...
    const fetch = ({ pageSize, page, sorted, filtered }) => {
        ...
        gridDataByClient({
            ...
            accessToken: user.accessToken,
            data: payload,
            apiname: "prodWarehouseLimitByProduct"
        })
            .then(res => {
                            ...
                            columns: [
                                {
                                    ...
                                    Cell: (row) => (
                                        <p className="gridBtn"
                                            onClick={() => {
                                                dispatch(productWarehouseLowLimitUpdate({ ...row.original }))
                                                props.goToUpdate()
                                            }}>{additionalsList.length > 0 && 
                                                additionalsList.filter(i => i.value == row.original.warehouse).length > 0 &&
                                                additionalsList.filter(i => i.value == row.original.warehouse)[0].label
                                                }  </p>
                                    )
                                },
                                {
