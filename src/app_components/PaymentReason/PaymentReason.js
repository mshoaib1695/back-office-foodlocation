...
import { paymentReasonUpdate } from '../../redux/actions/updatescreens/role'
...
function PaymentReason() {
    ...
    const fetch = ({ pageSize, page, sorted, filtered }) => {
        ...
        gridDataByClient({
            data: payload,
            apiname: "paymentReasonsByClient",
            tokenType: user.tokenType,
            accessToken: user.accessToken,
        })
            .then(res => { 
    ...
    return (
        <Card>
            <CardHeader>
                <CardTitle>Payment Reasons</CardTitle>
                <Button.Ripple
                    color="primary"
                    type="submit"
                    className="mr-1 mb-1"
                    onClick={e => history.push("/dashboard/createpaymentreason")}
                >
                            ...
                            columns: [
                                {
                                    ...
                                    Cell: (row) => (
                                        <p className="gridBtn"
                                            onClick={() => {
                                                dispatch(paymentReasonUpdate({ ...row.original }))
                                                history.push('/dashboard/updatepaymentreason')
                                            }}>
                                            {row.original.name}
                                        </p>
                                    )
