...
function Users(props) {
    ...
    const fetch = ({ pageSize, page, sorted, filtered }) => {
        ...
        gridDataByClient({
            data: payload,
            apiname: "stockMaintLinesByStockMaint",
            tokenType: user.tokenType,
            accessToken: user.accessToken,
        })
            .then(res => { 
    ...
    return (
        <Card>
            <CardHeader>
                {/* createmenuitem */}
                <CardTitle>Stock Maintainance Lines</CardTitle>
                <Button.Ripple
                    color="primary"
                    type="submit"
                    className="mr-1 mb-1"
                    ...
                    columns={[
                        {
                            columns: [
                                ...
                                },
                                {
                                    ...
                                    Cell: (row) => (
                                        <p className="gridBtn">
                                            {uomsList.length > 0 && uomsList.filter(i => i.value == row.original.baseUom).length > 0 &&
                                                uomsList.filter(i => i.value == row.original.baseUom)[0].label
                                            }
                                        </p>
                                    )
                                },
