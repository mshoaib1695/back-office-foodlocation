...
function ProductPrice(props) {
    ...
    const productIngredient = useSelector(state => state.updatescreens.productIngredient)    

    useEffect(() => {
        getUomsList()
        getIngredientProductsList()
        setBaseuom(productIngredient.uom)
    }, [])
    useEffect(() => {
        productIngdntSelect(productIngredient.ingrntProd,null,  productIngredient.qty)
    }, [ingredientProductsListt])
    ...
    const productIngdntSelect = (event, currentQTY, cc) => {
        ...
        if (ingredientProductsListt.length > 0) {
            if (ingredientProductsListt.filter(item => item.value == event).length > 0) {
                ...
                if (currentQTY && currentQTY.getFieldMeta("qty").value) {
                    costTemp = ingredientProductsListt.filter(item => item.value == event)[0].currentCost * currentQTY.getFieldMeta("qty").value
                    setCost(costTemp)
                }
                if(cc){
                    costTemp = ingredientProductsListt.filter(item => item.value == event)[0].currentCost * cc
                    setCost(costTemp)
                }
            }
        }
    ...
    return (
        <Card>
            <CardHeader>
                <CardTitle>Update Product Ingredients</CardTitle>
            </CardHeader>
            <CardBody>
                <Formik
                    initialValues={{
                        ingrntProd: productIngredient.ingrntProd,
                        qty: productIngredient.qty,
                        uom: productIngredient.uom,
