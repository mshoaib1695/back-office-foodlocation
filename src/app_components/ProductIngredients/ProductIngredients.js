...
import { productIngredientUpdate } from '../../redux/actions/updatescreens/role'
...
function ProductIngredients(props) {
    ...
    return (
        <Card>
            <CardHeader>
                <CardTitle>Product Ingredient</CardTitle>
                {<Button.Ripple
                    color="primary"
                    type="submit"
                    className="mr-1 mb-1"
                            ...
                            columns: [
                                {
                                    ...
                                    Cell: (row) => (
                                        <p className="gridBtn"
                                            onClick={() => {
                                                dispatch(productIngredientUpdate({ ...row.original }))
                                                props.goToUpdate()
                                            }}>
                                            {ingredientProductsListt.filter(i => i.value == row.original.ingrntProd).length > 0 && 
                                            ingredientProductsListt.filter(i => i.value == row.original.ingrntProd)[0].label }
                                        </p>
                                    )
                                },
                                {
                                ...
                                },
                                {
                                    ...
                                    Cell: (row) => (
                                        <p className="gridBtn"
                                            ...
                                            { row.original.qty * get_currentCost(row.original) }
                                        </p>
                                    )
