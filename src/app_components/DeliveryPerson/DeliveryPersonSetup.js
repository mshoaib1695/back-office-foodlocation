...
import { deliveryPersonUpdate } from '../../redux/actions/updatescreens/role'
...
function PaymentReason() {
    ...
    const fetch = ({ pageSize, page, sorted, filtered }) => {
        ...
        gridDataByClient({
            data: payload,
            apiname: "deliveryPersonsByClient",
            tokenType: user.tokenType,
            accessToken: user.accessToken,
        })
            .then(res => { 
        ...
        <Card>
            <CardHeader>
                ...
                <Button.Ripple
                    ...
                    type="submit"
                    className="mr-1 mb-1"
                    onClick={e => history.push("/dashboard/createdeliveryperson")}
                >
                    Create Package
                  </Button.Ripple>
            </CardHeader>
                            ...
                            columns: [
                                {
                                    ...
                                    Cell: (row) => (
                                        <p className="gridBtn"
                                            onClick={() => {
                                                dispatch(deliveryPersonUpdate({ ...row.original }))
                                                history.push('/dashboard/updatedeliveryperson')
                                            }}>
                                            {row.original.name}
                                        </p>
                                    )
